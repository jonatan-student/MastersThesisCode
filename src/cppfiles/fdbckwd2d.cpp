...
#define HELP ("Usage:  dphidx = fdbckwd2d(phi, dx, direction)\n")


DEFUN_DLD(fdbckwd2d, args, ,HELP){
	octave_value_list retval;

	if ( args.length() != 3 ) {
		error("fdbckwd2d: Incorrect input\n");
		return retval;
	}
	...
	if ( strcmp(dir.c_str(), "x")==0 ){
		for ( size_t j=1; j<ngrdy-1; j++ )
			for ( size_t i=1; i<ngrdx-1; i++ ) 
				dphi(j,i) = idx*(phi(j, i) - phi(j,i-1));
	}
	else if  ( strcmp(dir.c_str(), "y")==0 ){
		for ( size_t i=1; i<ngrdx-1; i++ )
			for ( size_t j=1; j<ngrdy-1; j++ ) 
				dphi(j,i) = idx*(phi(j, i) - phi(j-1,i));
	}
